        ...
        if (flags & OGS_PFCP_MODIFY_ACTIVATE)
        {
                ...
                (flags & OGS_PFCP_MODIFY_N2_HANDOVER))
            {
                ...
                r = smf_sbi_discover_and_send(
                    ...
                    sess, stream,
                    flags & OGS_PFCP_MODIFY_XN_HANDOVER ? SMF_UPDATE_STATE_ACTIVATED_FROM_XN_HANDOVER : SMF_UPDATE_STATE_ACTIVATED_FROM_N2_HANDOVER,
                    NULL);
                ogs_expect(r == OGS_OK);
            ...
            else if (flags & OGS_PFCP_MODIFY_DL_ONLY)
            {
                ...
                if (HOME_ROUTED_ROAMING_IN_VSMF(sess))
                {
                    ...
                    r = smf_sbi_discover_and_send(
                        ...
                        smf_nsmf_pdusession_build_hsmf_update_data,
                        sess, stream,
                        flags & OGS_PFCP_MODIFY_FROM_ACTIVATING ? SMF_UPDATE_STATE_ACTIVATED_FROM_ACTIVATING : SMF_UPDATE_STATE_ACTIVATED_FROM_NON_ACTIVATING,
                        NULL);
                    ogs_expect(r == OGS_OK);
                    ogs_assert(r != OGS_ERROR);
                }
    ...
    ogs_pfcp_session_report_request_t *pfcp_req)
{
    ...
    if (report_type.downlink_data_report)
    {
        ...
        {
            if (pfcp_req->downlink_data_report.downlink_data_service_information.presence)
            {
                info = pfcp_req->downlink_data_report.downlink_data_service_information.data;
                if (info)
                {
