...
static void pfcp_recv_cb(short when, ogs_socket_t fd, void *data)
{
    ...
    case OGS_PFCP_STATUS_NODE_ID_OPTIONAL_ABSENT:
        ogs_debug("ogs_pfcp_extract_node_id() "
                  ...
                  pfcp_status == OGS_PFCP_STATUS_SUCCESS ? ogs_pfcp_node_id_to_string_static(&node_id) : "NULL",
...
static void sess_timeout(ogs_pfcp_xact_t *xact, void *data)
{
    ...
    case OGS_PFCP_BLOCKCHAIN_NODE_ID_REQUEST_TYPE:
        ogs_error("No PFCP blockchain node id response");
        break;    
    ...
    upf_sess_t *sess, ogs_pfcp_blockchain_data_t *blockchain)
{
    ...
    // Prepare TLV for login
    login_tlv.presence = 1;
    login_tlv.len = blockchain->login_len;
    login_tlv.data = blockchain->login;

    // Prepare TLV for password
    pass_tlv.presence = 1;
    pass_tlv.len = blockchain->password_len;
    pass_tlv.data = blockchain->password;
