                                             ...
                                             const char *json_payload)
{
    ...
    // Length of the IP packet (data carried by GTP-U)
    size_t inner_packet_len = ip_hdr_len + udp_hdr_len + json_payload_len;
    
    // This size is confirmed to be 12 bytes due to the included extension fields
    size_t gtpu_hdr_len = sizeof(ogs_gtp1_header_t); 
    
    // GTP-U Length field = Inner IP Packet Length + Length of Extension fields (4 bytes)
ogs_pkbuf_t *ogs_gtpu_form_json_udp_packet(ogs_pkbuf_pool_t *pool,
                                             uint32_t teid,
                                             uint8_t qfi,
                                             uint32_t src_ip,
                                             ...
                                             const char *json_payload)
{
    ...
    // GTP-U Length field = Inner IP Packet Length + Length of Extension fields (4 bytes)
    size_t gtpu_length_field = inner_packet_len + 4;
    ...
    /* ---------------- 2. GTP-U Header (12 Bytes) ---------------- */
    ogs_gtp1_header_t *gtp_h = (ogs_gtp1_header_t *)current_ptr;
    memset(gtp_h, 0, sizeof(*gtp_h));

    gtp_h->version = OGS_GTP1_VERSION_1; // 1
    gtp_h->pt = 1;                       // Protocol Type (GTP)
    gtp_h->e = 1;                        // **E flag set: Extension Header is present**
    gtp_h->s = 0;                        // Sequence Number not present
    gtp_h->pn = 0;                       // N-PDU Number not present
    gtp_h->type = OGS_GTP1U_MSGTYPE_GPDU; // 255
    
    // Set total payload length (IP packet + 4 bytes extension fields)
    gtp_h->length = htons(gtpu_length_field); 
    gtp_h->teid = htonl(teid);
    
    // Extension Header Fields (4 bytes due to struct definition):
    gtp_h->sqn = 0; 
    gtp_h->npdu = 0; 
    
    // **Set Next Extension Header Type to 0 (No Next Extension)**
    // The standard QFI extension requires an additional byte for the QFI itself,
    // which would make the header 13 bytes. Setting the next header type to 0
    // tells the receiver to stop reading extensions after these 4 bytes.
    // NOTE: If QFI is *required* by the gNB/UE, this might need further modification
    ...
    // **CRITICAL FIX: Set Version (4) and IHL (5) in the first byte (0x45)**
    uint8_t version_ihl = (4 << 4) | (ip_hdr_len / 4); 
