...
ogs_sbi_request_t *ogs_sbi_build_request(ogs_sbi_message_t *message)
{
    ...
    if (message->param.home_plmn_id_presence)
    {
        ...
        if (home_plmn_id.mnc && home_plmn_id.mcc)
        {
            ...
            if (home_plmn_id.mnc)
                ogs_free(home_plmn_id.mnc);
            if (home_plmn_id.mcc)
                ogs_free(home_plmn_id.mcc);
            ...
            if (v)
            {
                ...
                if (item)
                {
                    ...
                    if (SliceInfoForPduSession)
                    {
                        ...
                        message->param.slice_info_for_pdu_session_presence = true;
...
void ogs_sbi_header_free(ogs_sbi_header_t *h)
{
    ...
    if (h->method)
        ogs_free(h->method);
    if (h->service.name)
        ogs_free(h->service.name);
    if (h->api.version)
        ogs_free(h->api.version);

    for (i = 0; i < OGS_SBI_MAX_NUM_OF_RESOURCE_COMPONENT &&
                h->resource.component[i];
         i++)
    ...
    ogs_plmn_id_t *target_plmn_id)
{
    ...
    memcpy(&discovery_option->target_plmn_list[discovery_option->num_of_target_plmn_list],
    ...
    ogs_plmn_id_t *requester_plmn_id)
{
    ...
    memcpy(&discovery_option->requester_plmn_list[discovery_option->num_of_requester_plmn_list],
